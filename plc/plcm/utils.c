...
#define CT_FLOAT    4   /* %g conversion */

const unsigned char *__sccl(char *, const unsigned char *);
		...
		switch (c) {
            ...
            case CT_FLOAT:
                ...
                for (p = buf; width; width--) {
				    c = *inp;
				    /*
				     * Switch on the character; `goto ft_ok'
				     * if we accept it as a part of number.
				     */
                    switch (c) {
				        /* 0 through 9 always legal */
		    ...
		    ft_ok:
			    ...
			    /*
			     * If we had only a sign, point or e it is no good;
			     * push back the char.  If the number ends in `e'
                 * or '.' push back the char 
			     */
			    if (flags & NDIGITS) {
				    if (p > buf) {
					    inp--;
...
unsigned long long int _strtoull(const char *ptr, char **endptr, int base)
{
...
skip0x:
  ...
  while(*nptr) {
    ...
    if (c>=base) break;	/* out of base */
    {
      ...
      register unsigned long long w=(v/256)*base+(x/256);
      //if (w>(ULLONG_MAX>>8)) overflow=1;
      if (w>(ULLONG_MAX/256)) overflow=1;
      //v=(w<<8)+(x&0xff);
      v=(w*256)+(x&0xff);
    }
    ++nptr;
